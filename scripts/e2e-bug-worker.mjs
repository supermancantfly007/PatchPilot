...
const apiPort = Number(process.env.PATCHPILOT_E2E_BUG_PORT || 4250 + (process.pid % 1000));
...
const afterRepro = await poll(async () => {
  ...
  return { currentBug, fixWorkItem, reproRun, reproTestCase };
}, 15000);
...
async function withApiServer(run) {
  ...
  const dataDir = await mkdtemp(join(tmpdir(), "patchpilot-bug-e2e-"));
