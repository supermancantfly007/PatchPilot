...
async function runSmokeScenario(scenario) {
  ...
  try {
    ...
    await waitForUsableAction(addLink, `${scenario.name}: add intake link`);
    await addLink.click();
    ...
    const metricsRegion = page.getByRole("region", { name: "成本和产品指标" });
    await metricsRegion.getByText("需求到 PR 时间").waitFor();
    await metricsRegion.getByText("自主完成率").waitFor();
    await metricsRegion.getByText("首次测试通过率").waitFor();
    await metricsRegion.getByText("人类介入次数").waitFor();
    await metricsRegion.getByText("成本 / 接受 PR").waitFor();
    await metricsRegion.getByText("复现成功率").waitFor();
    await metricsRegion.getByText("审计完整率").waitFor();
    await metricsRegion.getByText("返工轮次").waitFor();
    await metricsRegion.getByText("最终接受率").waitFor();
    await metricsRegion.getByText(/[1-9]\d*\/[1-9]\d* 个已结束 run 成功/).waitFor();
    await metricsRegion.getByText(/[1-9]\d*\/[1-9]\d* 个工作项的最新验收决策为接受/).waitFor();
    await metricsRegion.getByText("暂无失败原因").waitFor();
